...
**Run a 30B Model on an 8 GB Laptop GPU** — measured at 53.8 tok/s. With reproducible commands and an explanation of *why* every flag exists, not just *how* to use it.

**What's Inside:**
- 📘 **LESSONS_LEARNED.md** — Feynman-style theory: VRAM hierarchy, MoE, quantization, KV cache, picking inference tools.
...
- 🎯 **DIFFUSIONGEMMA.md** — **Jev vs DiffusionGemma vs Qwen (+ Ternary Bonsai 27B, Laya), measured.** Bonsai is 5.9 GB, fits entirely on an 8 GB GPU, and is about as accurate as Jev on these tasks. Laya, a 421M open "Jev alternative", answers in 10–60 ms but is narrow. TypeSafe's Jev: flat 0.34 s, 94 % / 85 %, $0.014 per 1,000 decisions. **DiffusionGemma as a Jev-style decision engine on an 8 GB laptop:** 0 output tokens, 20 answers in one pass. Calibrated yes/no, choice and score answers from one bidirectional pass. Includes a llama.cpp backend for OpenJev, a 400-question benchmark against Qwen 3 30B-A3B (89 % vs 83 %, 0 vs 31 format errors), and a claims audit of the viral "~0.2 s, zero hallucinations" post.
...
| Model | Speed | Why it matters |
|---|---:|---|
| **Qwen 3 30B-A3B MoE** | **53.8 tok/s** | A 30B model at near-chat speed via `-ncmoe` expert offload |
| Qwen 3 8B + TurboQuant @ 32K | 28.5 tok/s | Long context unlocked — FP16 KV cache OOMs at 32K |

**Hardware Focus:** AMD Ryzen AI 9 365 + RTX 5060 Laptop GPU (8 GB, Blackwell sm_120). Built on llama.cpp from source + Madreag's TurboQuant fork.
