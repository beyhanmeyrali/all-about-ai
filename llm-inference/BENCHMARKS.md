...
| **Ternary Bonsai 2 27B** (dense, hybrid attn, Qwen3.8-27B base) | **PTQ1_0** (1.75 bpw ternary) | **5.53 GiB** | **99 (all)** | **347.0 ± 2.2** | **31.7 ± 0.0** | **A dense 27B entirely in 8 GB VRAM.** 4× the tg of Qwen3.6-27B at Q3 (7.8). Needs [PrismML's llama.cpp fork](https://github.com/PrismML-Eng/llama.cpp) (`9a9394a`); `-fa 1`, 3 reps. |
...
- **Ternary Bonsai 2 27B at 31.7 t/s, with the whole model on the GPU**: a *dense* 27B (Qwen3.8-27B base) compressed to ternary weights {−1, 0, +1} at 1.75 bits each, in a 5.9 GB file. It's the first 27B-class model in this table that doesn't spill into system RAM. That's why it's 4× faster than the Qwen3.6-27B Q3 row (7.8 t/s, which had to offload), though still slower than the 3B-active MoEs (a dense model runs all 27B weights per token). It isn't just small; it's good: on the decision tests in [DIFFUSIONGEMMA.md](DIFFUSIONGEMMA.md) it scored **92.5 % / 86.5 %**, against Qwen 3 30B-A3B's 83 % / 65.5 %, with the best calibration of any language model tested, including TypeSafe's Jev (only the small Laya classifier, asked as a choice, scored lower).
...
Bonsai 27B tern ████████████████ 31.7                   ← dense 27B at 1.75 bits: all 5.5 GiB on the GPU
