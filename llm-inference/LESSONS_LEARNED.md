...
| **Ryzen AI NPU (XDNA 2, ~50 TOPS)** | ❌ not by llama.cpp, ✅ by FastFlowLM running concurrently — see [NPU.md](NPU.md) | llama.cpp has no NPU backend (the request is [open upstream](https://github.com/ggml-org/llama.cpp/issues/14377)). But FastFlowLM gives us 92.6 tok/s on Qwen 3 0.6B at ~+10 W incremental — and crucially the dGPU stays 0 % util while NPU works, so it's a true *concurrent* third accelerator. |
...
**They don't have to stay idle.** With a separate llama.cpp build using `-DGGML_VULKAN=ON` and a small-model GGUF, the iGPU can run a parallel inference target on a different port — useful for embeddings, vision, or a fast helper LLM while the dGPU is busy with a 30B MoE. The NPU runs the same kind of small-model + embedding workload via **FastFlowLM** at higher tokens-per-watt and zero contention with the dGPU — see **[NPU.md](NPU.md)** for the install (one `.deb`, no reboot) and measured numbers. **[HARDWARE_BEYOND_CUDA.md](HARDWARE_BEYOND_CUDA.md)** orients you across the iGPU + NPU + ROCm landscape.
