...
| **[SERVING_AT_SCALE.md](SERVING_AT_SCALE.md)** | **Why serving an LLM is an operating-systems problem.** The KV-cache memory wall (one H100 holds ~29 conversations), PagedAttention as virtual memory, continuous batching as preemptive scheduling, prefill/decode disaggregation — plus a claims audit correcting a viral version of this story. Ships `kv_cache_calc.py`. | When you wonder how ChatGPT-scale actually works. |
| **[LOCAL_CLUSTER.md](LOCAL_CLUSTER.md)** | **Buyer's guide to a local LLM cluster** — recommendations first, then the reasoning. Whether you need a cluster at all, the three numbers that decide it (capacity / bandwidth / latency), sparse vs dense, pipeline vs tensor splitting, every option priced (used 3090s, Threadripper builds, Mac Studio, Mac mini clusters, DGX Spark, Strix Halo), the traps, and running costs. | Before you spend money on hardware. |
| **[DIFFUSIONGEMMA.md](DIFFUSIONGEMMA.md)** | **Jev vs DiffusionGemma vs Qwen: fast AI decisions, measured.** Speed, tokens, cost, and accuracy for TypeSafe's Jev (cloud) and two local models. **DiffusionGemma as a Jev-style decision engine on 8 GB.** What TypeSafe's Jev and the open OpenJev are, how a "System One read" gets calibrated answers out of one bidirectional pass, a llama.cpp backend so it runs with expert offload (`diffusiongemma/`), a 400-question benchmark against Qwen 3 30B-A3B, and a claims audit of the viral post. | When you need an LLM to *decide* (classify, route, score) rather than write. |
...
| **DiffusionGemma 26B-A4B** as a decision engine (Q4) | 17 GB | **534 ms / decision** | Jev-style reads: 89 % SST-2 / 77 % AG News, 0 output tokens, 20 answers in 363 ms. The real Jev (cloud): 94 % / 85 %, flat 0.34 s, $0.014 per 1,000 decisions. Text generation only 14.6 tok/s. |
